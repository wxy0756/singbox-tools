...
if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -q 'agsb/sing-box' || pgrep -f 'agsb/sing-box' >/dev/null 2>&1; then
    if [ "$1" = "rep" ]; then
        [ "$vlr" = yes ] || [ "$vmp" = yes ] || [ "$trp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] || [ "$anyp" = yes ]  || { echo "提示：rep重置协议时，请在脚本前至少设置一个协议变量哦，再见！💣"; exit; }
    fi
else
    if [ "$1" != "del" ]; then
        [ "$vlr" = yes ] || [ "$vmp" = yes ] || [ "$trp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] || [ "$anyp" = yes ]  || { echo "提示：未安装agsb脚本，请在脚本前至少设置一个协议变量哦，再见！💣"; exit; }
    fi
fi
...
export any_sni=${any_sni:-"www.bing.com"}   # Default SNI for AnyTLS protocol
...
EOF
   ...
   # 生成私钥
    ...
    if [ -n "$anyp" ]; then
        if [ -z "$port_anytls" ] && [ ! -e "$HOME/agsb/port_anytls" ]; then
            port_anytls=$(shuf -i 10000-65535 -n 1)
            echo "$port_anytls" > "$HOME/agsb/port_anytls"
        elif [ -n "$port_anytls" ]; then
            echo "$port_anytls" > "$HOME/agsb/port_anytls"
        fi

        port_anytls=$(cat "$HOME/agsb/port_anytls")
        yellow "AnyTLS端口：$port_anytls"

        # AnyTLS password (建议 base64)，如未指定则自动生成并落盘
        # AnyTLS password 直接复用 UUID（与其它协议保持一致）
        anytls_password="$uuid"

        cat >> "$HOME/agsb/sb.json" <<EOF
{"type": "anytls", "tag": "anytls-sb", "listen": "::", "listen_port": ${port_anytls}, "users": [ { "name": "anytls", "password": "${anytls_password}" } ], "tls": { "enabled": true, "insecure": true, "alpn": ["h2", "http/1.1"], "certificate_path": "$HOME/agsb/anytls_cert.pem", "key_path": "$HOME/agsb/anytls_private.key", "server_name": "${any_sni}" }},
EOF
    fi
...
vl_sni=\"${vl_sni}\"  tu_sni=\"${tu_sni}\"  hy_sni=\"${hy_sni}\"  any_sni=\"${any_sni}\" \
...
$tup=\"${port_tu}\" $anyp=\"${port_anytls}\"  $vlr=\"${port_vlr}\" \
...
write2AgsbFolders(){
    ...
    echo "${any_sni}" > "$HOME/agsb/any_sni"
...
cip(){
    ...
    if grep -q "anytls-sb" "$HOME/agsb/sb.json"; then
       ...
       # anytls_link="anytls://${uuid}:${password}@${server_ip}:${port_anytls}?sni=${any_sni}&security=tls&alpn=h3&allow_insecure=1#${sxname}anytls-$hostname"
        anytls_link="anytls://${password}@${server_ip}:${port_anytls}?sni=${any_sni}&alpn=h2,http/1.1&allow_insecure=true#${sxname}anytls-$hostname"
        ...
        surge_anytls_link="${sxname}anytls-$hostname = anytls, ${server_ip}, ${port_anytls}, password=${uuid}, sni=${any_sni}, alpn=h2,http/1.1, allow_insecure=true"
...
if [ "$1" = "rep" ]; then cleandel; rm -rf "$HOME/agsb"/{sb.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name,short_id,cdn_host,hy_sni,vl_sni,tu_sni,any_sni,port_anytls,anytls_pass}; echo "重置完成..."; sleep 2; fi
